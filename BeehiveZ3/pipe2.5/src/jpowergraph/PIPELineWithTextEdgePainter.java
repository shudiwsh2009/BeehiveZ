...
/**
 * This class defines the edge painter for edges with text used in PIPE Change:
 * - the arrow placement
 * 
 * @author Pere Bonet
 */
public class PIPELineWithTextEdgePainter<T extends TextEdge> extends
		LineWithTextEdgePainter<T> {
	...
	public void paintEdge(JGraphPane graphPane, JPowerGraphGraphics g, T edge,
		...
		g.setForeground(getEdgeColor(edge, graphPane, false,
				theSubGraphHighlighter));
		g.setBackground(getEdgeColor(edge, graphPane, false,
				theSubGraphHighlighter));
		...
		if (slope > 2.0) {
			...
			midpoint.setLocation(midpoint.getX() + xDiff, midpoint.getY()
					+ yDiff);
		} else if (slope < -2.0) {
			double xDiff = 10;
			double yDiff = -10;
	...
	public static void paintArrow(JPowerGraphGraphics g, int x1, int y1,
			int x2, int y2, boolean isDashedLine) {
		double middleX = (x1 + 2 * x2) / 3;
		double middleY = (y1 + 2 * y2) / 3;

		double slope = Math.atan2(y2 - y1, x2 - x1);

		double pinnacleX = middleX + SQUARE_ROOT_OF_3_OVER_2
				* ARROW_BASE_LENGTH * Math.cos(slope);
		double pinnacleY = middleY + SQUARE_ROOT_OF_3_OVER_2
				* ARROW_BASE_LENGTH * Math.sin(slope);
		double backwardX = pinnacleX + ARROW_BASE_LENGTH
				* Math.cos(slope + Math.PI + Math.PI / 6.0);
		double backwardY = pinnacleY + ARROW_BASE_LENGTH
				* Math.sin(slope + Math.PI + Math.PI / 6.0);
		double forwardX = pinnacleX + ARROW_BASE_LENGTH
				* Math.cos(slope + Math.PI - Math.PI / 6.0);
		double forwardY = pinnacleY + ARROW_BASE_LENGTH
				* Math.sin(slope + Math.PI - Math.PI / 6.0);
		...
		g.fillPolygon(new int[] { (int) backwardX, (int) backwardY,
				(int) pinnacleX, (int) pinnacleY, (int) forwardX,
				(int) forwardY });
	}
