...
public abstract class AbstractBranchingProcess<BPN extends IBPNode<N>, C extends ICondition<BPN, C, E, F, N, P, T, M>, E extends IEvent<BPN, C, E, F, N, P, T, M>, F extends IFlow<N>, N extends INode, P extends IPlace, T extends ITransition, M extends IMarking<F, N, P, T>>
		...
		if (this.ca.get(n2) == null) {
			if (n2 instanceof AbstractEvent) {
				...
				if (c.getPreEvent().equals(n1))
					return true;
				if (this.ca.get(c.getPreEvent()).contains(n1))
					return true;
	...
	public boolean areConcurrent(BPN n1, BPN n2) {
		...
		if (notCo != null)
			if (notCo.contains(n2))
				return false;

		boolean result = !this.areCausal(n1, n2)
				&& !this.areInverseCausal(n1, n2)
				&& !this.areInConflict(n1, n2);
	...
	public boolean areInConflict(BPN n1, BPN n2) {
		...
		for (BPN nn1 : ca1) {
			...
			for (BPN nn2 : ca2) {
				...
				if (e1.equals(e2))
					continue;
				if (!this.overlap(e1.getPreConditions(), e2.getPreConditions()))
					continue;
	...
	@SuppressWarnings("unchecked")
	public OrderingRelationType getOrderingRelation(E e1, E e2) {
		return getOrderingRelation((BPN) e1, (BPN) e2);
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public E createEvent(T transition,
	...
	public boolean appendEvent(E event) {
		...
		// add conditions that correspond to post-places of transition that
		// corresponds to new event
		ICoSet<BPN, C, E, F, N, P, T, M> postConditions = null;
		try {
			postConditions = (ICoSet<BPN, C, E, F, N, P, T, M>) AbstractCoSet.class
					.newInstance();
	...
	private boolean overlap(ICoSet<BPN, C, E, F, N, P, T, M> s1,
			ICoSet<BPN, C, E, F, N, P, T, M> s2) {
		for (C n : s1)
			if (s2.contains(n))
				return true;
		...
		for (C c1 : this.conds) {
			for (C c2 : this.conds) {
				...
				if (c1.getPlace().equals(c2.getPlace())
						&& this.areConcurrent((BPN) c1, (BPN) c2))
					return false;
			}
		}
		return true;
	...
	public IOccurrenceNet<BPN, C, E, F, N, P, T, M> getOccurrenceNet() {
		try {
			@SuppressWarnings("unchecked")
			IOccurrenceNet<BPN, C, E, F, N, P, T, M> occ = (IOccurrenceNet<BPN, C, E, F, N, P, T, M>) OccurrenceNet.class
					.newInstance();
	...
	public IBranchingProcess<BPN, C, E, F, N, P, T, M> clone() {
		...
		try {
			clone = (IBranchingProcess<BPN, C, E, F, N, P, T, M>) BranchingProcess.class
					.newInstance();
		} catch (InstantiationException e) {
			e.printStackTrace();
		} catch (IllegalAccessException e) {
	...
	/*
	 * protected boolean appendEvent2(Event e) { this.events.add(e);
	 * this.updateCausalityEvent(e);
	 * 
	 * if (t2es.get(e.getTransition())!=null)
	 * t2es.get(e.getTransition()).add(e); else { Set<Event> es = new
	 * HashSet<Event>(); es.add(e); t2es.put(e.getTransition(), es); }
	 * 
	 * // add conditions that correspond to post-places of transition that
	 * corresponds to new event CoSet postConds = new CoSet(this.sys); //
	 * collection of new post conditions for (Place s :
	 * this.sys.getPostset(e.getTransition())) { // iterate over places in the
	 * postset Condition c = new Condition(s,e); // construct new condition
	 * postConds.add(c); this.addCondition(c); // add condition to unfolding }
	 * e.setPostConditions(postConds); // set post conditions of event
	 * 
	 * // compute new cuts of unfolding for (Cut cut :
	 * c2cut.get(e.getPreConditions().iterator().next())) { if
	 * (contains(cut,e.getPreConditions())) { Cut newCut = new
	 * Cut(this.sys,cut); newCut.removeAll(e.getPreConditions());
	 * newCut.addAll(postConds); if (!this.addCut(newCut)) return false; } }
	 * 
	 * this.countEvents++; return true; }
	 */
}
