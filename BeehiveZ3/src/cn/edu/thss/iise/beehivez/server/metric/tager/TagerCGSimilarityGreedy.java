...
public class TagerCGSimilarityGreedy extends PetriNetSimilarity {
...
//		GEDAbstr measure = new GEDAlgorithmAStar();
		...
		float sim = TagerCGSimilarityGreedy.similairity(measure, tg1, tg2,
				WEIGHT_SKIPPED_EDGE, WEIGHT_SKIPPED_VERTEX);
		if (Math.abs(sim - (-1)) < EPSILON) {
			measure = new GEDAlgorithmGreedy();
			measure.setWeight(weights);
			sim = TagerCGSimilarityGreedy.similairity(measure, tg1, tg2,
					WEIGHT_SKIPPED_EDGE, WEIGHT_SKIPPED_VERTEX);
		}
		return sim;
	}
	...
	public static float similairity(TLabeledGraph tg1, TLabeledGraph tg2) {
		...
		LabelEditDistance.scaleWeight = SCALE_WEIGHT;
		measure.setWeight(weights);
		return TagerCGSimilarityGreedy.similairity(measure, tg1, tg2,
				WEIGHT_SKIPPED_EDGE, WEIGHT_SKIPPED_VERTEX);
	}
	...
	public static void singleExperiment() {
		try {
			...
			Object[] weights = new Object[10];
			weights[0] = "vweight";
			weights[1] = TagerCGSimilarityGreedy.WEIGHT_SKIPPED_VERTEX;
			weights[2] = "sweight";
			weights[3] = TagerCGSimilarityGreedy.WEIGHT_SUBSTITUTED_VERTEX;
			weights[4] = "eweight";
			weights[5] = TagerCGSimilarityGreedy.WEIGHT_SKIPPED_EDGE;
			weights[6] = "simcutoff";
			weights[7] = TagerCGSimilarityGreedy.SIM_CUTOFF;
			weights[8] = "waiting";
			weights[9] = 60000.0;
			LabelEditDistance.scaleWeight = TagerCGSimilarityGreedy.SCALE_WEIGHT;
...
//							TagerCGSimilarity.WEIGHT_SKIPPED_VERTEX));
			System.out.println("AStar: "
					+ TagerCGSimilarityGreedy.similairity(gedAStar, p, q,
							TagerCGSimilarityGreedy.WEIGHT_SKIPPED_EDGE,
							TagerCGSimilarityGreedy.WEIGHT_SKIPPED_VERTEX));
