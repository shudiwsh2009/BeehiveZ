	...
	public BehavioralRelationBuilder(ONCompleteFinitePrefix cfp){
		...
		for(int i = 0; i < length; ++i){
			for(int j = 0; j < length; ++j){
				this._relation[i][j] = BehavioralRelation.Undefined;
			}
		}
	}
...
//		}
				...
				for(int j = 0; j < length; ++j) {
					if(this._relation[i][j] == BehavioralRelation.StrictOrder) {
						for(int k = 0; k < length; ++k) {
							if((this._relation[j][k] == BehavioralRelation.StrictOrder)
									&& this._relation[i][k] == BehavioralRelation.Undefined) {
								this._relation[i][k] = BehavioralRelation.StrictOrder;
								change = true;
							}
						}
					}
		...
		for(int i = 0; i < length; ++i) {
			for(int j = 0; j < length; ++j) {
				if(this._relation[i][j] == BehavioralRelation.StrictOrder
						&& this._relation[j][i] == BehavioralRelation.StrictOrder) {
					this._relation[i][j] = this._relation[j][i]
							= BehavioralRelation.Interleaving;
				} else if(this._relation[i][j] == BehavioralRelation.StrictOrder) {
					this._relation[j][i] = BehavioralRelation.ReverseOrder; 
				}
			}
		}
		for(int i = 0; i < length; ++i) {
			for(int j = 0; j < length; ++j) {
				if(this._relation[i][j] == BehavioralRelation.Undefined) {
					this._relation[i][j] = BehavioralRelation.Exclussive; 
				}
			}
		}
