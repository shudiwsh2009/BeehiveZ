...
public class TagerTriangleEquationBatchLocal {
...
//	public static String ROOT_FOLDER = "C:\\Users\\Shudi\\Desktop\\tager\\";
	...
	public static void main(String[] args) throws Exception {
		...
		TagerTriangleEquationBatchLocal tb = new TagerTriangleEquationBatchLocal();
	...
	public void computeBatch(String algo) throws Exception{
		...
		if(algo.equals("greedy")) {
			...
			csvWriter = new BufferedWriter(new FileWriter(rootFolder + "triangle_equation_local_tager_greedy.csv"));
		} else if(algo.equals("astar")) {
			measure = new GEDAlgorithmAStar();
			csvWriter = new BufferedWriter(new FileWriter(rootFolder + "triangle_equation_local_tager_astar.csv"));
		} else {
			System.out.println("Unsupported algorithm");
			return;
		}
		FileUtils.createFolder(meetFolder);
		csvWriter.write("simcutoff,k,weightSkippedVertex,weightSubstitutedVertex,weightSkippedEdge,"
				+ "Local_time,Local4-2");
		csvWriter.newLine();
		...
		tGraphs = loadFiles(files);
		double[][] simMatrix = new double[tGraphs.size()][tGraphs.size()];
			...
			for(double scaleWeight : scaleWeights) {
				for(int i = 1; i <= 10; ++i) {
					...
					for(int j = 1; j <= 10; ++j) {
						...
						String parameters = "simcutoff=" + simCutoff + ",k=" + scaleWeight 
								+ ",weightSkippedVertex=" + weightSkippedVertex 
								+ ",weightSubstitutedVertex=" + weightSubstitutedVertex 
								+ ",weightSkippedEdge=" + weightSkippedEdge;
						System.out.println(parameters);
						csvWriter.write("," + generateSimMatrix(simMatrix, measure, tGraphs, weightSkippedEdge, weightSkippedVertex));
						csvWriter.write("," + meet(simMatrix, tGraphs, "Local", parameters, meetFolder));
						csvWriter.newLine();
					}
				}
			}
	...
	private double meet(double[][] simMatrix, List<TLabeledGraph> tGraphs,
			String dataset, String parameters, String meetFolder) throws IOException {
		...
		if(builder4_2.length() != 0) {
			BufferedWriter writer = new BufferedWriter(new FileWriter(meetFolder
					+ parameters + "-" + dataset + ".txt"));
...
//							+ simpleCGs.get(p).getFilename() + " & " + simpleCGs.get(q).getFilename());
					long a = System.currentTimeMillis();
					simMatrix[p][q] = simMatrix[q][p]
							= TagerCGSimilarity.similairity(measure, tGraphs.get(p), tGraphs.get(q),
