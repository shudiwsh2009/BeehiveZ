...
public class UndoManager {
	...
	public void redo() {
		...
		if (undoneEdits > 0) {
			...
			Iterator<UndoableEdit> currentEdit = edits.get(indexOfNextAdd)
					.iterator();
	...
	private void deleteObject(PetriNetObject pnObject) {
		if (pnObject instanceof ArcPathPoint) {
			...
			if (pnObject instanceof PlaceTransitionObject) {
				//
				Iterator arcsTo = ((PlaceTransitionObject) pnObject)
				...
				//
				Iterator arcsFrom = ((PlaceTransitionObject) pnObject)
				...
				if (((NormalArc) pnObject).hasInverse()) {
					if (((NormalArc) pnObject).hasInvisibleInverse()) {
						...
						addEdit(new DeletePetriNetObjectEdit(inverse, view,
								model));
						inverse.delete();
					} else {
						addEdit(((NormalArc) pnObject).clearInverse());
