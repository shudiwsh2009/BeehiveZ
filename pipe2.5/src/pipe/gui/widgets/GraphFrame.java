...
public class GraphFrame extends JFrame {
	...
	public void constructGraphFrame(DefaultGraph graph, String markingLegend) {
		this.setIconImage(new ImageIcon(CreateGui.imgPath + "icon.png")
				.getImage());
		this.graph = graph;
		...
		jGraphPane.setNodePainter(PIPETangibleState.class, PIPETangibleState
				.getShapeNodePainter());

		jGraphPane.setNodePainter(PIPEInitialTangibleState.class,
				PIPEInitialTangibleState.getShapeNodePainter());

		jGraphPane.setNodePainter(PIPEVanishingState.class, PIPEVanishingState
				.getShapeNodePainter());
		...
		jGraphPane.setNodePainter(PIPEState.class, PIPEState
				.getShapeNodePainter());

		jGraphPane.setNodePainter(PIPEInitialState.class, PIPEInitialState
				.getShapeNodePainter());
		...
		jGraphPane.setDefaultEdgePainter(new LineEdgePainter(
				JPowerGraphColor.BLACK, JPowerGraphColor.GRAY, false));
		...
		jGraphPane.setPopupDisplayer(new SwingPopupDisplayer(
				new PIPESwingToolTipListener(),
				new PIPESwingContextMenuListener(graph, new LensSet(),
						new Integer[] {}, new Integer[] {})));

		Layouter m_layouter = new Layouter(new SpringLayoutStrategy(graph));
		m_layouter.start();

		SwingJGraphScrollPane scroll = new SwingJGraphScrollPane(jGraphPane,
				lensSet);

		SwingJGraphViewPane view = new SwingJGraphViewPane(scroll, lensSet,
				true);
